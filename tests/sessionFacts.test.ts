...
describe('extractSessionFacts', () => {
  ...
  it('does not use local-command caveat messages as the goal', () => {
    ...
    const facts = extractSessionFacts([
      user(caveat),
      user('<command-name>/login</command-name><command-message>login</command-message>'),
      user('fix the login redirect bug in the auth module'),
    ], 's1', 'proj', 0);
    expect(facts.goal).toBe('fix the login redirect bug in the auth module');
  });
